...
/*
 * File Overview:
 * Use Case: Date/slot selection, affordability guard aur booking confirm flow handle karta hai.
 * Project Role: Booking UX ka interactive core component hai.
 * Trigger: Interviewer profile page me booking section open hone par.
 * File Path: app/(main)/interviewers/[id]/_components/SlotPicker.jsx
 */
...
export default function SlotPicker({
  ...
  // Auto-advance to the first date that has available future slots
  // so users see 45-min slots by default instead of an empty "Today" tab.
  useEffect(() => {
    if (!availability) return;
    // Note: default selected date ko first "bookable" day par le jaate hain,
    // warna user ko often empty today tab dikh sakta hai.
  ...
  // Note: affordability gate - insufficient credits pe booking confirm nahi karne dena.
  ...
  useEffect(() => {
    if (data?.success && data.streamCallId) {
      // Note: booking create hote hi user ko appointments listing pe bhej dete hain.
  ...
  const handleSlotClick = (slot) => {
    ...
    if (!canAfford) {
      // Note: low credits case me direct error dene ke badle upgrade modal open karte hain.
      setUpgradeOpen(true);
      return;
    }
  ...
  const handleConfirm = () => {
    ...
    // Note: selected Date objects ko server action ke liye ISO string me convert karna जरूरी है.
    bookFn({
