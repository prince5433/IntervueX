...
/*
 * File Overview:
 * Use Case: Category-based AI interview questions generate/show karta hai.
 * Project Role: Interviewer ko live prompt support deta hai call ke beech me.
 * Trigger: Call UI me AI tab select karne par.
 * File Path: app/(main)/call/[callId]/_components/AIQuestions.jsx
 */
...
export default function AIQuestionsPanel({ categories }) {
  const availableCategories = useMemo(() => {
    ...
    // Fallback keeps the feature usable even for older interviewer profiles
    // that may not have categories populated.
    return ["FULLSTACK"];
  }, [categories]);
  ...
  useEffect(() => {
    setSelectedCategory((prev) =>
      availableCategories.includes(prev) ? prev : availableCategories[0]
    );
  }, [availableCategories]);
    ...
    <div className="flex flex-col gap-4 h-full overflow-hidden">
      ...
      {(!Array.isArray(categories) || categories.length === 0) && (
        <p className="text-[11px] text-stone-600">
          Category profile missing. Using Full Stack as default.
        </p>
      )}
