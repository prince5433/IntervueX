...
/*
 * File Overview:
 * Use Case: Live call UI: video pane, chat pane, AI questions tab aur leave controls.
 * Project Role: Interview session ka real-time interaction surface yahi hai.
 * Trigger: CallRoom ke andar successful join ke baad render.
 * File Path: app/(main)/call/[callId]/_components/CallUI.jsx
 */
...
export default function CallUI({
  ...
  const handleLeave = useCallback(async () => {
    try {
      if (call) {
        ...
        if (isRecording) {
          // Note: interviewer/user abrupt leave par recording abruptly कट na ho,
          // isliye pehle explicit stop attempt.
  ...
  // Note: Stream ecosystem me same user token ko chat + video dono me reuse kar rahe hain.
  ...
  useEffect(() => {
    ...
    // Note: callId ko channel id bana ke same interview ke liye persistent thread maintain hota hai.
    ...
    channel
      ...
      .catch(console.error);
    return () => {
      // Note: page leave par channel watch cleanup.
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            {activeTab === "chat" ? (
              ...
              <div className="p-4 h-full overflow-y-scroll max-h-screen">
                <AIQuestionsPanel
                  categories={booking.categories ?? booking.interviewer?.categories}
                />
              </div>
            )}
          </div>
