/*
 * File Overview:
 * Use Case: Earnings stats, withdrawal modal aur payout history render karta hai.
 * Project Role: Interviewer monetization workflow ka UI/control center hai.
 * Trigger: Dashboard earnings tab open hone par.
 * File Path: app/(main)/dashboard/components/EarningsSection.jsx
 */
    ...
    if (data?.success) {
      setEmailIssue(
        data?.emailSent
          ? ""
          : data?.emailError ||
              "Withdrawal request created, but confirmation email could not be sent."
      );
      setDone(true);
      setTimeout(() => {
        setOpen(false);
        ...
        <DialogContent className="bg-[#0f0f11] border border-white/10 text-stone-100 max-w-md">
          {done ? (
            <div className="py-8 text-center flex flex-col items-center gap-4">
              ...
              {emailIssue && (
                <p className="text-xs text-amber-400 max-w-xs">{emailIssue}</p>
              )}
            </div>
          ) : (
            <>
