...
export function createRateLimiter({ refillRate, interval, capacity }) {
  return arcjet({
    ...
    // Note: IP ke badle userId fingerprint use kar rahe hain,
    // taaki shared network/IP scenarios me valid users unfairly block na hon.
...
export async function checkRateLimit(aj, req, userId) {
  ...
  if (decision.isDenied()) {
    // Note: reason-type ke basis par user message customize karte hain.
