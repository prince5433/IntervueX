...
const shouldAllocateCredits = (dbUser, currentPlan) => {
  // Case 0: Credits are null/0 or missing allocation date -> allocate immediately
  if ((dbUser.credits ?? 0) === 0 || !dbUser.creditsLastAllocatedAt) return true;

  // Case 1: Plan changed (upgrade/downgrade) → allocate immediately
  if (dbUser.currentPlan !== currentPlan) return true;

  // Case 2: New calendar month has started since last allocation
  const now = new Date();
