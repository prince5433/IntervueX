...
Analyze the candidate's performance. Respond ONLY with a valid JSON object, no markdown, no backticks, no explanation:
{
  ...
  // Parse Gemini's JSON response — wrap in try-catch for proper error handling
  let feedbackData;
  try {
    feedbackData = JSON.parse(raw);
  } catch (parseErr) {
    console.error("[feedback] Failed to parse Gemini JSON:", parseErr.message, "Raw:", raw.slice(0, 200));
    return { status: "parse-error" };
  }

  // Earning credit check — dedupe on (bookingId, type) BEFORE transaction
  // to avoid unique constraint violations
  const earnExists = await db.creditTransaction.findFirst({
    where: { bookingId: booking.id, type: "BOOKING_EARNING" },
  });

  // Atomic transaction: feedback upsert + booking status + earning credit
  // All three must succeed together or none — prevents partial state
  await db.$transaction([
    ...
    // Interviewer earning sirf ek baar credit karna hai — only if not already created
    ...(earnExists
      ? []
      : [
