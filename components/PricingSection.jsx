...
/*
 * File Overview:
 * Use Case: Plan cards, active plan detection aur checkout actions render karta hai.
 * Project Role: Subscription conversion aur plan management UI ka main block hai.
 * Trigger: Landing/pricing/upgrade modal me render hone par.
 * File Path: components/PricingSection.jsx
 */
      ...
      {PLANS.map((plan) => {
        ...
        return (
          ...
          >
            ...
            {isActive ? (
            ...
            ) : isSignedIn && billingEnabled && !!plan.planId ? (
              <CheckoutButton
                planId={plan.planId}
                planPeriod="month"
                checkoutProps={{
            ...
            ) : isSignedIn && billingEnabled ? (
              <Button
                variant={plan.featured ? "gold" : "outline"}
                className="w-full"
                disabled
                title="Set NEXT_PUBLIC_CLERK_STARTER_PLAN_ID and NEXT_PUBLIC_CLERK_PRO_PLAN_ID"
              >
                Plan unavailable
              </Button>
            ) : isSignedIn ? (
              <Button
                ...
                className="w-full"
                disabled
                title="Set NEXT_PUBLIC_CLERK_BILLING_ENABLED=true after enabling Clerk Billing"
              >
                Billing unavailable
              </Button>
            ) : (
